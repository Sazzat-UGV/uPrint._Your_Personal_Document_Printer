...
@section('content')
    @include('backend.layout.inc.breadcumb', [
        'main_page' => 'Departments',
        'sub_page' => 'Department List',
    ])
                ...
                <tbody>
                    @foreach ($departments as $index => $department)
                        <tr>
                            ...
                            <td>
                                <div class="form-check form-switch ">
                                    <input class="form-check-input toggle-class-subject" type="checkbox" role="switch"
                                        data-id="{{ $department->id }}" id="department-{{ $department->id }}"
                                        {{ $department->add_subject ? 'checked' : '' }}>
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item"
                                            href="{{ route('department.edit', $department->slug) }}"><i
...
@push('admin_script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.toggle-class-subject').change(function() {
                var add_subject = $(this).prop('checked') == true ? 1 : 0;
                var dept_id = $(this).data('id');
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '/admin/check/department/add_subject/' + dept_id,
                    success: function(response) {
                        console.log(response);
                        Swal.fire(
                            'Status Updated!',
